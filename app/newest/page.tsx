        ...
        <div className="retro-container" style={{ padding: "4px 8px", borderBottom: "1px solid #e0e0e0", fontSize: "8pt" }}>
          {(categories as any[]).map((c: any, i: number) => (
            <span key={c.id}>
              ...
              <Link href={`/categories?cat=${c.slug}`} style={{ color: "#828282" }}>
                {c.name}
              </Link>
            </span>
          ))}
      ...
      <div className="retro-container">
                ...
                <tr key={p.id} style={{ borderBottom: "1px solid #eee" }}>
                  ...
                  <td style={{ verticalAlign: "top", paddingTop: 6, paddingBottom: 6 }}>
                    <div>
                      ...
                      {p.screenshot_url && (
                        <Link href={`/product/${p.slug}`} style={{ marginLeft: 4, fontSize: "7pt", color: "#828282" }}>
                          [img]
                        </Link>
                      )}
                    </div>
                    <div className="retro-meta" style={{ marginTop: 2 }}>
                      {p.upvotes} points ·{" "}
                      <Link href={`/categories?cat=${p.category_slug}`} style={{ color: "#ff6600" }}>
                        {p.category_name}
                      </Link>{" "}
                      · by <Link href={sellerLink}>{sellerDisplay}</Link> · {timeAgo(p.created_at)} ago ·{" "}
                      <Link href={`/product/${p.slug}#reviews`} style={{ color: "#828282" }}>
                        reviews
                      </Link>{" "}
                      ·{" "}
                      <Link href={`/product/${p.slug}#comments`} style={{ color: "#828282" }}>
                        comments
                      </Link>
                    </div>
                  </td>
        ...
        {(products as any[]).length === 0 && (
          <div style={{ textAlign: "center", padding: 40, color: "#828282" }}>
            No products yet.{" "}
            <Link href="/submit" style={{ color: "#ff6600" }}>
        ...
        <div style={{ padding: "8px 0", borderTop: "1px solid #e0e0e0", marginTop: 4, fontSize: "9pt" }}>
          <Link href="/" style={{ color: "#828282" }}>
            ← Top / popular
          </Link>
        </div>

        <div style={{ borderTop: "2px solid #ff6600", marginTop: 12, padding: "6px 0", fontSize: "8pt", color: "#828282" }}>
          <Link href="/about">About</Link> |<Link href="/faq"> FAQ</Link> |<Link href="/api"> API</Link> |
